...
def process_outputs_gaussian(outputs, extra: List[Dict] = None):
    ...
    If extra has 'teacher_predictions_lp' then access these.
    """
    if extra is None: return outputs, extra

    check = 'teacher_predictions_lp' in extra[0]

    # Returns a list of predictions
    if not check: return outputs, extra
    ...
    # A list of dictionaries for scale of gaussian
    extra = [
        {'student_predictions_scale': ex['student_predictions_scale'][:, i]}
        for ex in extra
        for i in range(num)
    ]
