...
@dataclass
class KLDivergenceAndGaussCriterionConfig(FairseqDataclass):
    ...
    temperature_scale_est: float = field(
        default=1.0,
        metadata={"help": "temperature scaling teacher predictions for gaussian student"}
    )
...
@register_criterion("kl_divergence_distillation_and_gauss", dataclass=KLDivergenceAndGaussCriterionConfig)
class KLDivergenceAndGaussCriterion(KLDivergenceCriterion):
    def __init__(
            self,
            task,
            sentence_avg,
        ...
        super(KLDivergenceAndGaussCriterion, self).__init__(
            task = task,
    ...
    @classmethod
    def build_criterion(cls, cfg: KLDivergenceAndGaussCriterionConfig, task):
        return KLDivergenceAndGaussCriterion(
            task = task,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        gauss_loss = self.compute_dir_loss(model, net_output, sample, reduce) if model.training else 0.0
